	...
	SubShader
	{
		Pass{
			...
			float4x4 unity_MatrixMVP;
				...
				inline float4 ComputeScrennPos(float4 pos)
				{
					float4 o = pos * 0.5f;
				...
				fixed4 frag(v2f i) : SV_TARGET
				{
					...
					float offset_positive = +ceil(_Thinkness * 0.5f);
					float offset_negative = -floor(_Thinkness * 0.5f);
