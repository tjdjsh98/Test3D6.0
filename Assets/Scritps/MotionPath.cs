...
public class MotionPath_Editor : Editor
{
    ...
    {
        string OutString = "";
        AnimatorController animatorController = GetAnimator.runtimeAnimatorController as AnimatorController;
        AnimatorOverrideController animatorOverrideController = GetAnimator.runtimeAnimatorController as AnimatorOverrideController;
        if(animatorController)
        {
            AllState = GetAnimatorStates(animatorController); //모든 스테이트 가져오기   
        }
        if(animatorOverrideController)
        {
            AllState = GetAnimatorStates(animatorOverrideController); //모든 스테이트 가져오기   
        }
        Debug.Log(AllState);
        Debug.Log(Clip);
    ...
    AnimatorState[] GetAnimatorStates(AnimatorOverrideController aniOverride)
    {
        List<AnimatorState> ret = new List<AnimatorState>();
        AnimatorController anicon = aniOverride.runtimeAnimatorController as AnimatorController;
        foreach (var layer in anicon.layers)
        {
            foreach (var subsm in layer.stateMachine.stateMachines)
            {
    ...
    AnimatorState GetStateFromClip(AnimatorState[] StateList, AnimationClip GetClip)
    {
        ...
        Debug.Log(StateList.Length);
        for (int i = 0; i < StateList.Length; i++)
        {
            Debug.Log(StateList[i].motion);
